...
def get_mamba_layer(
    ...
    mamba_layer = MambaLayer(input_dim=in_channels, output_dim=out_channels)  # 输入通道数加倍了
    if stride != 1:  # 步长不为1则添加池化层
        if spatial_dims == 2:   # 2D数据
            return nn.Sequential(mamba_layer, nn.MaxPool2d(kernel_size=stride, stride=stride))
        if spatial_dims == 3:   # 3D数据
...
class ResMambaBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = get_mamba_layer(   # 定义第一个Mamba层
            spatial_dims, in_channels=in_channels, out_channels=in_channels
        )
        self.conv2 = get_mamba_layer(   # 定义第二个Mamba层
            spatial_dims, in_channels=in_channels, out_channels=in_channels
        )
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)   # 对输入数据进行归一化
        # x = self.act(x) # 激活
        x = self.conv1(x)   # 使用第一个Mamba层
        x = self.act(x)  # 激活
        # 对输出进行归一化并激活
        x = self.norm2(x)
        # x = self.act(x)
        x = self.conv2(x)   # 使用第二个Mamba层
...
class ResUpBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm2 = get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=in_channels)
        self.act = get_act_layer(act)
        self.conv = get_dwconv_layer(  # 定义一个深度可分离卷积层
...
class LightMUNet(nn.Module):
    ...
    def __init__(
            ...
            blocks_down: tuple = (1, 2, 2, 4),  # 下采样残差块数，原本是只有3个层次，但是最后一个4表示有4个RVM层作为瓶颈层编码器
            blocks_up: tuple = (1, 1, 1),   # 上采样残差块数
    ...
    def _make_down_layers(self):
        ...
        """
        down_layers = nn.ModuleList()   # 创建一个空的ModuleList
        ...
        for i, item in enumerate(blocks_down):  # 遍历blocks_down列表：blocks_down: tuple = (1, 2, 2, 4)
            ...
            # get_maml_layer(item)返回的是一个完整的RVM层
            downsample_mamba = (
                get_mamba_layer(spatial_dims, layer_in_channels // 2, layer_in_channels, stride=2)  # 此时的stride=2表示下采样为2倍
                if i > 0
                else nn.Identity()  # 如果是第一层不进行下采样，此处做法是一个恒等变换，输入就是输出，相当于占位
            )
            # 创建一个包含downsample_mamba和item个ResMambaBlock的Sequential对象
            down_layer = nn.Sequential(
                downsample_mamba,   # 除了第一层之外每一层的Encoder Block都先进行一步下采样
                *[ResMambaBlock(spatial_dims, layer_in_channels, norm=norm, act=self.act) for _ in range(item)] # 每一个Encode Block都包含item个ResMambaBlock，每个ResMambaBlock包含两个RVM块，这里不进行下采样
            )
    ...
    def _make_up_layers(self):
        ...
        for i in range(n_up):   # 解码器除了DWConv外有3层
            ...
            # 创建一个包含DWConv层以及Interpolation层的Sequential对象
            up_samples.append(
                nn.Sequential(
                    *[
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        # 编码部分，处理输入张量并返回编码结果和中间特征图
        x = self.convInit(x)    # 初始卷积,上面的深度可分离卷积

        if self.dropout_prob is not None:   # dropout
            x = self.dropout(x)
        down_x = [] # 中间特征图
...
if __name__ == '__main__':
    ...
    '展示LightM-Unet结构'
    net = LightMUNet().to(device='cuda:1')
...
# !PYTHONWARNINGS="ignore" python LightMUNet.py
