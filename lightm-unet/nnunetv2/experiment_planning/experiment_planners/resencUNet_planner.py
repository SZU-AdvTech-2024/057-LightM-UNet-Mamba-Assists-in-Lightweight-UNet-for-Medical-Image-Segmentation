...
if __name__ == '__main__':
    ...
    # print(net.compute_conv_feature_map_size((64, 64, 64)))  # -> 558319104. The value you see above was finetuned
    ...
    net = ResidualEncoderUNet(input_channels=1, n_stages=7, features_per_stage=(32, 64, 128, 256, 512, 512, 512),
                              ...
                              nonlin=nn.LeakyReLU, nonlin_kwargs={'inplace': True}, deep_supervision=True)
    print(net.compute_conv_feature_map_size((128, 64)))  # -> 129793792计算给定输入尺寸 (512, 512) 经过网络的所有卷积层后，最终特征图的大小

    torch.onnx.export(net, torch.randn(1, 1, 128, 64), 'resencUNet.onnx')
    netron.start("resencUNet.onnx")
    print('done')
# 继承自 ExperimentPlanner 类。这个文件的主要目的是配置和初始化一个基于残差编码器 U-Net（Residual Encoder UNet）的实验计划
# 初始化参数：
# 在 __init__ 方法中，设置了多个与 Residual Encoder UNet 相关的参数，包括基础特征数、参考值、每阶段的块数等。
# 网络架构的配置：
# 配置了 Residual Encoder UNet 的各种参数，如输入通道数、阶段数、每阶段的特征数、卷积操作、核大小、步幅、归一化操作、激活函数等。
